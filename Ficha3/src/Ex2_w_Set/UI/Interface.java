package Ex2_w_Set.UI;

import Ex2_w_Set.Logic.*;
import java.util.*;

public class Interface {
    String[] options = {"Add Library", "Add Book", "Remove Library", "Remove Book", "List Lib", "List Books", "Exit"};
    Set<Library> store;
    ...
    public boolean menu() {
        ...
        switch (opt) {
            ...
            case 2: {
                ...
                opt = getOpt();
                getLib(--opt).addBook(new Book(name, aut));
                break;
            }
            case 3: {
                clearScreen();
    ...
    public Library getLib(int index) {
        Iterator<Library> it = store.iterator();
        while(it.hasNext()) {
            if(it.next().getCode_lib() == index)
                return it.next();
        }
        return null;
    }
